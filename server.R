...
function(input, output, session) {
  ...
  output$RepartitionConso <- renderPlotly({
    ...
    if (input$type_logement_choix2 != "Tout") {
      Logements=  subset(logementsSansVal,Type_bâtiment == input$type_logement_choix2)}
    #DPE
    if (input$DPE_choix2 != "Tout") {
      Logements=  subset(logementsSansVal,Etiquette_DPE == input$DPE_choix2)}
    #Code
    if (input$code_postal_choix2 != "Tout") {
      Logements=  subset(logementsSansVal,`Code_postal_(BAN)` == input$code_postal_choix2)}
