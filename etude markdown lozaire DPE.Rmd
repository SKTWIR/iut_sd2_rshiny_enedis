---
title: "Étude DPE des habitations du Lozère"
...
La répartition des étiquettes DPE de ces habitions est ainsi constitué :
...
couleurs <- c("A" = "#519740",  # Bleu pour A
              "B" = "#33cc33",  # Orange pour B
              "C" = "#ccff33",  # Vert pour C
              "D" = "#ffff00",  # Rouge pour D
              "E" = "#ffcc00",  # Violet pour E
              "F" = "#ff9a33",  # Marron pour F
              "G" = "#ff0000")  # Rose pour G
...
ggplot(compte_df_2, aes(x = Etiquette_DPE, y = nombre, fill = Etiquette_DPE)) +
  ...
  labs(title = "Répartition des étiquettes DPE",
       x = "Étiquette DPE",
       y = "Nombre d'appartement") +
  scale_fill_manual(values = couleurs) +  # Appliquer la palette de couleurs personnalisée
...
- 1% des logement qui sont étiquetés DPE A
- 3% des logement qui sont étiquetés DPE B
- 7% des logement qui sont étiquetés DPE C
- 26% des logement qui sont étiquetés DPE D
- 30% des logement qui sont étiquetés DPE E
- 17% des logement qui sont étiquetés DPE F
- 15% des logement qui sont étiquetés DPE G

Près d'1/3 des logements de notre CSV sont considéré comme étant une **passoire thermique**, cela signifie qu'ils ne peuvent être mis à la vente ou a la location sans travaux effectué au préalable.
...
Nous pouvons aussi nous rendre compte de la différence de performance entre les différentes types de logement avec les étiquette DPE. le cout moyen du chauffage est relativement le même entre les étiquettes A et D après cela, les coûts explose
...
## Existe-t-il un lien entre émission de GES le cout annuel

Comme nous l'avons vu, le cout annuel moyen d'un appartement en Lozère est de 2014.66€, mais nous avons vu que la performance énergétique attribué avec les étiquettes DPE joue un rôle important dans le cout annuel des 5 usages d'un logements.
Nous pouvons vérifier cela avec le graphique ci-dessous, qui est une régression linéaire.
On remarque que le coefficient de corrélation est de 0.86 ce qui signifie qu'il y a une importante influence entre le cout annuel et les émissions de GES émises par un logement.
...
Comme nous l'avons vu plus l'étiquette DPE est basse plus le coût annuel est important, grâce à cette régression linéaire et au coefficient de corrélation nous pouvons le prouver